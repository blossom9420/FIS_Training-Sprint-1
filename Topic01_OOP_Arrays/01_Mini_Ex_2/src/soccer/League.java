        ...
        for (Team homeTeam: theTeams){
            for (Team awayTeam: theTeams){
               if (homeTeam != awayTeam) {
                   //daysBetweenGames += 7; old
                   daysBetweenGames += 4;
                   /* Practice 11-2. Increment daysBetweenGames here */
                   // theGames.add(new Game(homeTeam, awayTeam, LocalDateTime.now())); // new
                   /* Practice 11-2. Modify the statement below to add pass LocalDateTime into constructor */
                   //theGames.add(new Game(homeTeam, awayTeam)); old
                   theGames.add(new Game(homeTeam, awayTeam,
                           LocalDateTime.now().plusDays(daysBetweenGames))); // newest
               } 
...
//                thePeriod.getDays() + " day(s)\n";
        ...
        stringBuffer.append("The League is scheduled to run for ");
        stringBuffer.append( thePeriod.getMonths() + " month(s), and ");
        stringBuffer.append( thePeriod.getDays() + " day(s)\n");
